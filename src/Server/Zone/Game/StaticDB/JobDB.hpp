...
public:
	...
	void load_job_internal(libconfig::Setting &job_c, job_db_data &data, std::string job_name = "");
	void load_hp_sp_table(libconfig::Setting &cfg, job_db_data &data, std::string &job_name, std::string table_name);

private:
	std::unordered_map<std::string, job_db_data> _job_db;
};
}
}
}
