...
elseif packet_version >= 20080102 then
	local pid = 0x01fe
elseif packet_version >= 20040426 then
	local pid = 0x01fe
elseif packet_version >= 0 then
	local pid = 0x01fe
end

local ZC_ACK_ITEMREPAIR = {
	_id = pid,
	_length = 0,
	_buffer = nil
}
...
function ZC_ACK_ITEMREPAIR.new(o)
	o = o or ZC_ACK_ITEMREPAIR
	setmetatable(o, self)
	self.__index = self
	return o
end

function ZC_ACK_ITEMREPAIR.serialize()

end

return ZC_ACK_ITEMREPAIR
