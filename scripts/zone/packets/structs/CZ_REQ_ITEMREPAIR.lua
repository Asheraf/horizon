...
elseif packet_version >= 20080102 then
	local pid = 0x01fd
elseif packet_version >= 20070528 then
	local pid = 0x01fd
elseif packet_version >= 20070507 then
	local pid = 0x01fd
elseif packet_version >= 20070102 then
	local pid = 0x01fd
elseif packet_version >= 20040423 then
	local pid = 0x01fd
elseif packet_version >= 0 then
	local pid = 0x01fd
end

local CZ_REQ_ITEMREPAIR = {
	_id = pid,
	_length = 0,
	_buffer = nil
}
...
function CZ_REQ_ITEMREPAIR.new(o)
	o = o or CZ_REQ_ITEMREPAIR
	setmetatable(o, self)
	self.__index = self
	return o
end

function CZ_REQ_ITEMREPAIR.deserialize(buf)

end

return CZ_REQ_ITEMREPAIR
