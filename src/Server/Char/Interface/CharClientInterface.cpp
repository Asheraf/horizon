...
#include "Server/Char/Packets/HC_ACCOUNT_ID.hpp"
...
#include "Server/Char/Packets/HC_NOTIFY_SECOND_PASSWD_UPDATE.hpp"
#include "Server/Char/Packets/HC_NOTIFY_SECOND_PASSWD_ERROR.hpp"
...
bool CharClientInterface::make_new_character(std::string name, uint8_t slot, uint32_t hair_color, uint32_t hair_style, uint32_t job_class, uint8_t gender)
{
	...
	std::string new_map = sChar->get_char_config().get_start_map();
	uint16_t x = sChar->get_char_config().get_start_x(), y = sChar->get_char_config().get_start_y();
	...
	for (int j = 0; j < sChar->get_char_config().get_start_item_count(); j++) {
		std::pair<uint32_t, uint32_t> p = sChar->get_char_config().get_start_item(j);
	...
	uint16_t start_zeny = sChar->get_char_config().get_start_zeny();
...
character_delete_result CharClientInterface::character_delete_soft(uint32_t character_id)
{
	...
	std::chrono::system_clock::time_point dt = t + std::chrono::seconds(sChar->get_char_config().get_character_deletion_time());
...
bool CharClientInterface::character_delete_birthdate(uint32_t character_id, std::string birthdate)
{
	...
	if (sChar->get_char_config().get_char_hard_delete()) {
...
bool CharClientInterface::character_delete_reserve(uint32_t character_id)
{
	...
	std::chrono::system_clock::time_point tp = std::chrono::system_clock::now() + std::chrono::seconds(sChar->get_char_config().get_character_deletion_time());
...
bool CharClientInterface::pincode_create(uint32_t account_id, char *pincode)
{
	...
	{
		std::shared_ptr<sqlpp::mysql::connection> conn = sChar->get_db_connection();
		std::chrono::system_clock::time_point tp = std::chrono::system_clock::now() + std::chrono::seconds(sChar->get_char_config().get_pincode_expiry());
...
bool CharClientInterface::pincode_verify(uint32_t account_id, char *pincode)
{
	...
	if (get_session()->get_session_data()._pincode_tries == sChar->get_char_config().get_pincode_retry()) {
#if CLIENT_TYPE == 'M' && PACKET_VERSION >= 20180124 \
|| CLIENT_TYPE == 'R' && PACKET_VERSION >= 20180124 \
|| CLIENT_TYPE == 'Z' && PACKET_VERSION >= 20180131
