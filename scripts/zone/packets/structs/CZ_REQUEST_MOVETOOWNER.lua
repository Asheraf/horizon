...
elseif packet_version >= 20080102 then
	local pid = 0x0234
elseif packet_version >= 20050425 then
	local pid = 0x0234
elseif packet_version >= 0 then
	local pid = 0x0234
end

local CZ_REQUEST_MOVETOOWNER = {
	_id = pid,
	_length = 0,
	_buffer = nil
}
...
function CZ_REQUEST_MOVETOOWNER.new(o)
	o = o or CZ_REQUEST_MOVETOOWNER
	setmetatable(o, self)
	self.__index = self
	return o
end

function CZ_REQUEST_MOVETOOWNER.deserialize(buf)

end

return CZ_REQUEST_MOVETOOWNER
