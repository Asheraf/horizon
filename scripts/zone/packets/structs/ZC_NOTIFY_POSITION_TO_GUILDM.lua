...
elseif packet_version >= 20080102 then
	local pid = 0x01eb
elseif packet_version >= 20031124 then
	local pid = 0x01eb
elseif packet_version >= 0 then
	local pid = 0x01eb
end

local ZC_NOTIFY_POSITION_TO_GUILDM = {
	_id = pid,
	_length = 0,
	_buffer = nil
}
...
function ZC_NOTIFY_POSITION_TO_GUILDM.new(o)
	o = o or ZC_NOTIFY_POSITION_TO_GUILDM
	setmetatable(o, self)
	self.__index = self
	return o
end

function ZC_NOTIFY_POSITION_TO_GUILDM.serialize()

end

return ZC_NOTIFY_POSITION_TO_GUILDM
