...
{
public:
	InterPacketHandler(std::shared_ptr<SocketType> socket, std::string const &password, inter_connect_client_types client_type)
	: PacketHandler<SocketType>(socket), _client_type(client_type), _inter_password(password)
	{
		initialize_handlers();
	}
	...
	{
#define HANDLER_FUNC(packet) this->add_packet_handler(packet, boost::bind(&InterPacketHandler::Handle_ ## packet, this, boost::placeholders::_1))
...
#undef HANDLER_FUNC
	...
	virtual bool sync_receive_from_inter(PacketBuffer &pkt_buf)
	{
		...
		if (!this->get_socket()->is_open()) {
			this->get_socket()->close_socket();
			return false;
		}

		recv_size = this->get_socket()->sync_read(buf, buf.get_remaining_space());

		if (!recv_size)
			return false;

		memcpy(&op_code, buf.get_read_pointer(), sizeof(uint16_t));
		PacketBuffer recv_pkt(op_code, buf.get_read_pointer(), recv_size);
		pkt_buf = recv_pkt;

		return true;
	}

	virtual void receive_and_handle(PacketBuffer &buf)
	{
		if (sync_receive_from_inter(buf))
			this->handle_received_packet(buf);
	}
	...
	virtual void Send_INTER_CONNECT_AUTH()
	{
		...
		this->send_synchronous_packet(send_buf, packet_len);
		receive_and_handle(recv_buf);
	}
	...
	virtual void Send_INTER_SESSION_SET(SessionData &session_data)
	{
		...
		session_data >> pkt.s;
		this->send_synchronous_packet(pkt);
	}
	 ...
	 */
	inter_connect_client_types get_client_type() { return _client_type; }
